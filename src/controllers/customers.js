...
async function getCustomerInquiryInfo(req, res) {
  const { id: customerId } = req.params;
  const customer = await CustomerModel.findById(customerId)
  .populate("inquiries", "createTime serviceTime address  contactNo  email name message serviceId tradiesId totalPrice accepted deleted")
    .exec();
  if (!customer) {
    return res.status(404).json("customer Not Found");
  }
...
module.exports = {
  ...
  addInquiryForCustomer,getCustomerInquiryInfo,
};
