...
async function addInquiry(req, res){
  const {zipCode, jobDateTime, contactNo, email, firstName, lastName, message, clientId, tradiesId, serviceId};
  const inquiry = new Inquiry({zipCode, jobDateTime, contactNo, email, firstName, lastName, message, clientId, tradiesId, serviceId});
  await inquiry.save();
  return res.status(201).json(inquiry);
}
...
async function acceptInquiry(req, res) {
  const { id } = req.params;
  const inquiry = await Order.findByIdAndUpdate(id, { accepted: true }).exec();
  if (!inquiry) {
    return res.status(404).json("This inquiry is not found!");
  }
  if (inquiry.accepted) {
  ...
  await inquiry.save();
  return res.json(`Inquiry accepted.`);
}
